  ...
  for (const agency of agencies) {
    try {
      ...
      if (filters.minFineAmount) {
        query += ` AND fine_amount >= ${filters.minFineAmount}`;
      }
      if (filters.maxFineAmount) {
        query += ` AND fine_amount <= ${filters.maxFineAmount}`;
      }
      
      // Date range filters
      if (filters.dateRange?.start) {
        query += ` AND event_date >= '${filters.dateRange.start}'`;
      }
      if (filters.dateRange?.end) {
        query += ` AND event_date <= '${filters.dateRange.end}'`;
      }
      ...
      if (filters.violationTypes && filters.violationTypes.length > 0) {
        ...
        query += ` AND violation_type IN (${types})`;
      }
      if (filters.excludeTypes && filters.excludeTypes.length > 0) {
        const types = filters.excludeTypes.map(t => `'${t.replace(/'/g, "''")}'`).join(',');
        query += ` AND violation_type NOT IN (${types})`;
      }
      
      // Only get violations without Bedrock content
      query += ` AND (bedrock_title IS NULL OR bedrock_description IS NULL)`;
