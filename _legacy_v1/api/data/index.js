...
async function getEntityData(pk) {
    ...
    // Bedrock content is written directly to violation items by bedrock-generator,
    // so no overlay query needed - it's already in feedRes.Items
    const violations = (feedRes.Items || []).map(item => {
        ...
        // Bedrock content should be in the violation item itself (from gold-sync/parquet-writer)
        // It's written directly to both Silver and DynamoDB by bedrock-generator, so no overlay needed
        const bedrockTitle = item.title_bedrock || item.bedrock_title;
        const bedrockDescription = item.description_bedrock || item.bedrock_description;
        const attribution = item.attribution;
        
        // Debug logging
        if (bedrockTitle) {
            console.log(`[getEntityData] Found bedrock content for ${item.violation_id}: title="${bedrockTitle.substring(0, 50)}..."`);
        }
        ...
        return {
            ViolationData: {
                ...
                rawData: {
                    ...
                    raw_title: item.raw_title || item.title || null,
                    raw_description: item.raw_description || item.raw_desc || item.description || null,
                    eventtitle: item.eventtitle || null,
                    final_narrative: item.final_narrative || null,
                    event: item.event || null,
                    description: item.description || null,
                    city: item.city || null,
                    state: item.state || null,
                    employer: item.company_name || item.company || null,
                    ...item // Include all other fields as fallback
                },
            ...
            ProcessedContent: bedrockTitle ? {
                title: bedrockTitle,
                explanation: bedrockDescription || item.description,
                attribution: attribution || 'Data synthesized by SirSluginston VentureOS'
            } : null,
            violation_id: item.violation_id || item.SK?.split('#').pop() || null
        };
    });
