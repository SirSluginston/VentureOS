...
async function getReviewQueue(limit = 10) {
    try {
        // Scan for Bedrock content that hasn't been reviewed
        // Look for items with SK='BEDROCK_CONTENT' and no reviewed_at field
        const result = await docClient.send(new ScanCommand({
            TableName: VIOLATIONS_TABLE,
            FilterExpression: "SK = :sk AND (attribute_not_exists(reviewed_at) OR reviewed_at = :empty)",
        ...
        const items = (result.Items || []).map(item => ({
            ...
            title_bedrock: item.title_bedrock || item.title,
            description_bedrock: item.description_bedrock || item.description,
            tags: item.tags || [],
            generated_at: item.generated_at || item.generatedAt,
            attribution: item.attribution || 'Generated by AWS Bedrock'
        }));
...
async function approveContent(pk, sk) {
    try {
        await docClient.send(new UpdateCommand({
            TableName: VIOLATIONS_TABLE,
            Key: { PK: pk, SK: sk },
            UpdateExpression: "SET reviewed_at = :date, attribution = :attr, #status = :status",
